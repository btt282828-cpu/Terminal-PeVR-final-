...
def build_html(df, rrg, alerts, breadth, risk, regime, buy, avoid, sources, fred, flow=None, bt=None,
    ...
    try:
        ...
        if spy_flow:
            ...
            si += (f"<div>SPY FLOW CMF <span style='color:{GRN if (_sc or 0) > 0 else RED}'>{(_sc if _sc is not None else 0):+.2f}</span>"
                   f" · OBV {'<span style=\"color:' + GRN + '\">↑ sobre media</span>' if _so else '<span style=\"color:' + RED + '\">↓ bajo media</span>'}"
